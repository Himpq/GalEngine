...
from .GalRegistry import GalGlobals
...
class SDLSurface:
    def __init__(self, size: Tuple[int, int], alpha=True, texture = None, orig_surface=None, colorkey=None):
    ...
    def blit(self, src_surface, dest, area=None):
        ...
        prev_target = sdl2.SDL_GetRenderTarget(self.renderer) # 保存当前渲染目标
        sdl2.SDL_SetRenderTarget(self.renderer, self.texture) # 切换渲染目标到本 texture
        sdl2.SDL_RenderCopy(self.renderer, src_surface.texture, srcrect, dstrect) # 执行拷贝
        sdl2.SDL_SetRenderTarget(self.renderer, prev_target) # 恢复渲染目标
    ...
    def copy(self):
        """
        返回一个新的 SDLSurface，内容与当前 surface 相同
        """
        prev_target = sdl2.SDL_GetRenderTarget(self.renderer) # 保存当前渲染目标
        ...
        sdl2.SDL_SetTextureBlendMode(new_texture, sdl2.SDL_BLENDMODE_BLEND)
        sdl2.SDL_SetRenderTarget(self.renderer, new_texture)
        sdl2.SDL_RenderCopy(self.renderer, self.texture, None, None)
        sdl2.SDL_SetRenderTarget(self.renderer, prev_target)
        return SDLSurface((self.width, self.height), texture=new_texture)
...
class draw:
    ...
    def line(target, color, start, end, width=1):
        ...
        for i in range(-half, half + 1):
            offset_x = int(round(nx * i))
            offset_y = int(round(ny * i))
            s0 = (start[0] + offset_x, start[1] + offset_y)
            s1 = (end[0] + offset_x, end[1] + offset_y)
            if hasattr(target, "texture") and hasattr(target, "renderer"):
                sdl2.SDL_SetRenderTarget(target.renderer, target.texture)
                r, g, b, a = color
                sdl2.SDL_SetRenderDrawColor(target.renderer, r, g, b, a)
                sdl2.SDL_RenderDrawLine(target.renderer, int(s0[0]), int(s0[1]), int(s1[0]), int(s1[1]))
                sdl2.SDL_SetRenderTarget(target.renderer, None)
            else:
                r, g, b, a = color
                sdl2.SDL_SetRenderDrawColor(target, r, g, b, a)
                sdl2.SDL_RenderDrawLine(target, int(s0[0]), int(s0[1]), int(s1[0]), int(s1[1]))
...
class image:
    def load(path, cache=False):
        ...
        """
        if ImageCachePool.get(path) and cache:
...
class Switcher:
    Font        = SDLFont if GalGlobals.usingSDL2 else pygame.font.Font
    SysFont     = SysFont if GalGlobals.usingSDL2 else pygame.font.SysFont
    smoothscale = scale   if GalGlobals.usingSDL2 else pygame.transform.scale
    scale       = scale   if GalGlobals.usingSDL2 else pygame.transform.scale
    flip        = flip    if GalGlobals.usingSDL2 else pygame.transform.flip
    rotate      = rotate  if GalGlobals.usingSDL2 else pygame.transform.rotate

    class draw:
        rect = draw.rect  if GalGlobals.usingSDL2 else pygame.draw.rect
        line = draw.line  if GalGlobals.usingSDL2 else pygame.draw.line

    class image:
        load = image.load if GalGlobals.usingSDL2 else pygame.image.load

    def Rect(x, y, w, h):
        return [x, y, w, h]
    
    def Surface(wh, alpha=pygame.SRCALPHA, *arg, **args):
        if GalGlobals.usingSDL2:
            return SDLSurface(wh, *arg, alpha=alpha, **args)
        else:
            return pygame.Surface(wh, alpha, *arg, **args)